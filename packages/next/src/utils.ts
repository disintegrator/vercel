  ...
  async (
    ...
    if (
      ...
      if (renderingMode === RenderingMode.PARTIALLY_STATIC) {
        ...
        // Add the headers to the chain.
        const headers: Record<string, string> = {};
        if (routesManifest?.ppr?.chain?.headers) {
          Object.assign(headers, routesManifest.ppr.chain.headers);
        }
        ...
        if (experimentalStreamingLambdaPaths) {
          ...
          outputPath = experimentalStreamingLambdaPath.output;
          if (experimentalStreamingLambdaPath.pathname) {
            headers['x-matched-path'] =
              experimentalStreamingLambdaPath.pathname;
          }
        } else {
