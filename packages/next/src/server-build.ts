...
export async function serverBuild({
  ...
  const supportsPPRChaining = typeof routesManifest?.ppr !== 'undefined';

  const experimentalStreamingLambdaPaths = supportsPPRChaining
    ? undefined
    : new Map<
        string,
        {
    ...
    for (const group of combinedGroups) {
      ...
      for (const pageFilename of group.pages) {
        ...
        if (isRoutePPREnabled) {
          ...
          // If we're still using the experimental streaming lambda path, then
          // we should add those keys to the map.
  ...
  if (isAppPPREnabled && experimentalStreamingLambdaPaths) {
    debug(
