...
export class Prerender {
  ...
  constructor({
    ...
    if (chain !== undefined) {
      ...
      if (chain.headers && typeof chain.headers !== 'object') {
        throw new Error(
      ...
      if (chain.outputPath && typeof chain.outputPath !== 'string') {
        throw new Error(
          'The `chain.output` argument for `Prerender` must be a string.'
        );
      }

      this.chain = chain;

      // Mirror the output name to the experimental streaming lambda path. This
      // can be removed once removal of this property has stabilized.
      this.experimentalStreamingLambdaPath = chain.outputPath;
    }
  }
}
